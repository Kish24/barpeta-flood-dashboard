...
# 3. SAFETY CHECKS: Resolve column names defensively
if 'sector_id' not in sectors.columns:
    if 'name' in sectors.columns:
        sectors['sector_id'] = sectors['name']
    elif 'id' in sectors.columns:
        sectors['sector_id'] = sectors['id']
    else:
        sectors['sector_id'] = [f"Sector_{i+1}" for i in range(len(sectors))]

if 'mean_priority' not in sectors.columns:
    if 'mean' in sectors.columns:
        sectors['mean_priority'] = sectors['mean']
    elif 'priority' in sectors.columns:
        sectors['mean_priority'] = sectors['priority']
    else:
        sectors['mean_priority'] = 0.25

if 'active_sar_pixels' not in sectors.columns:
    if 'active_flood_pixels' in sectors.columns:
        sectors['active_sar_pixels'] = sectors['active_flood_pixels'].astype(int)
    else:
        sectors['active_sar_pixels'] = 0

if 'tier' not in sectors.columns:
    sectors['tier'] = np.where(
        (sectors['active_sar_pixels'] > 0) | (sectors['mean_priority'] >= 0.30),
        1,
        2
    )

# 4. Prepare clean Pandas DataFrame for Folium Choropleth lookup
...
with col1:
    ...
    m = folium.Map(location=[26.32, 91.00], zoom_start=10, tiles='CartoDB positron')
